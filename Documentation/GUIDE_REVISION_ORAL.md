...
|---|---|---|---|
| 1 | Outils de transformation/nettoyage adaptés mobilisés efficacement | `Donnees/data_cleaning_report.py` (contrôle intégrité + doublons sur 8 099 images) + `Modeles/projet.ipynb` §1.1 (réorganisation SH17→YOLO) et §1.3 (resize, normalisation) | « J'ai écrit un script qui scanne les 8 099 images : 0 corrompue, 0 doublon exact. Le prétraitement (resize 640×640, normalisation) est géré nativement par YOLO, documenté section 2.4 du rapport. » |
